...
public static class AuthCertificates
{
    public static void Configure(OpenIddictServerBuilder server, IHostEnvironment environment, IConfiguration configuration)
    {
        ...
        server.AddSigningCertificate(Load("Signing"));
        server.AddEncryptionCertificate(Load("Encryption"));

        X509Certificate2 Load(string purpose)
        {
            var path = configuration[$"Certificates:{purpose}:Path"];
            if (string.IsNullOrWhiteSpace(path))
            {
                throw new InvalidOperationException($"Configure Certificates:{purpose}:Path outside Development. See README.md.");
            }
